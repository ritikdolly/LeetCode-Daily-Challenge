...
public class MaxDistanceBetwPointsonSquare {
    ...
    class Solution {
        ...
        private boolean isSatisfyManhattan(int threshold, int[][] sortedPts, int n, int k) {
            ...
            for (int i = 0; i < 2 * n; i++) {
                ...
                while (j < 2 * n) {
                    int dist = Math.abs(sortedPts[i][0] - sortedPts[j][0])
                            + Math.abs(sortedPts[i][1] - sortedPts[j][1]);
                    if (dist >= threshold) {
                        break;
                    }
                    j++;
    ...
    public static void main(String[] args) {
        MaxDistanceBetwPointsonSquare.Solution solution = new MaxDistanceBetwPointsonSquare().new Solution();
        int side = 4;
        int[][] points = {{0, 0}, {0, 4}, {4, 0}, {4, 4}};
        int k = 2;
        int result = solution.maxDistance(side, points, k);
